...
1. Setup the branch:
   ...
   ```
2. Make a small, safe change in `terraform/demo/terraform.tfvars` (e.g., `ec2_instance_type = "t3.small"`).
3. Commit and push:
   ```bash
   git add terraform/demo/terraform.tfvars
...
1. After you’re done with the first PR, create the second branch:
   ...
   ```
2. Make an expensive/risky change in `terraform/demo/terraform.tfvars` (e.g., `rds_instance_class = "db.r6g.2xlarge"`).
3. Commit and push:
   ```bash
   git add terraform/demo/terraform.tfvars
